...
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    // 会话key
    private final static String HDONG_UPMS_SHIRO_SESSION_ID = "hdong-upms-shiro-session-id";
    // 全局会话key
    ...
    // 局部会话key
    private final static String HDONG_UPMS_CLIENT_SESSION_ID = "hdong-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String HDONG_UPMS_CLIENT_SESSION_IDS = "hdong-upms-client-session-ids";

    @Override
    protected Serializable doCreate(Session session) {
        Serializable sessionId = generateSessionId(session);
        assignSessionId(session, sessionId);
        RedisUtil.set(HDONG_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(session), (int) session.getTimeout() / 1000);
    ...
    protected void doDelete(Session session) {
        ...
        if ("client".equals(upmsType)) {
            ...
            String code = RedisUtil.get(HDONG_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            Jedis jedis = RedisUtil.getJedis();
            jedis.del(HDONG_UPMS_CLIENT_SESSION_ID + "_" + sessionId);
            jedis.srem(HDONG_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId);
            jedis.close();
        }
        if ("server".equals(upmsType)) {
            // 当前全局会话code
            String code = RedisUtil.get(HDONG_UPMS_SERVER_SESSION_ID + "_" + sessionId);
            // 清除全局会话
            RedisUtil.remove(HDONG_UPMS_SERVER_SESSION_ID + "_" + sessionId);
            // 清除code校验值
            RedisUtil.remove(HDONG_UPMS_SERVER_CODE + "_" + code);
            // 清除所有局部会话
            Jedis jedis = RedisUtil.getJedis();
            Set<String> clientSessionIds = jedis.smembers(HDONG_UPMS_CLIENT_SESSION_IDS + "_" + code);
            for (String clientSessionId : clientSessionIds) {
                jedis.del(HDONG_UPMS_CLIENT_SESSION_ID + "_" + clientSessionId);
                jedis.srem(HDONG_UPMS_CLIENT_SESSION_IDS + "_" + code, clientSessionId);
            }
            _log.debug("当前code={}，对应的注册系统个数：{}个", code, jedis.scard(HDONG_UPMS_CLIENT_SESSION_IDS + "_" + code));
            jedis.close();
            // 维护会话id列表，提供会话分页管理
            RedisUtil.lrem(HDONG_UPMS_SERVER_SESSION_IDS, 1, sessionId);
        }
        // 删除session
        RedisUtil.remove(HDONG_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    public Map<String, Object> getActiveSessions(int offset, int limit) {
        ...
        // 获取当前页会话详情
        List<String> ids = jedis.lrange(HDONG_UPMS_SERVER_SESSION_IDS, offset, (offset + limit - 1));
        List<Session> rows = new ArrayList<>();
        for (String id : ids) {
            String session = RedisUtil.get(HDONG_UPMS_SHIRO_SESSION_ID + "_" + id);
            // 过滤redis过期session
            if (null == session) {
                RedisUtil.lrem(HDONG_UPMS_SERVER_SESSION_IDS, 1, id);
                total = total - 1;
                continue;
            }
    ...
    public int forceout(String ids) {
        ...
        for (String sessionId : sessionIds) {
            ...
            RedisUtil.set(HDONG_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int) upmsSession.getTimeout() / 1000);
        }
